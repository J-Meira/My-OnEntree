...
const localAccessToken = useCookies.get('SG_AT_EP');
...
export const refresh = (): AppThunk => (dispatch) => {
  dispatch(setLoading('refresh'));
  authServices.refresh().then((r) => {
    dispatch(removeLoading('refresh'));
    if (r) dispatch(authSlice.actions.setAuth(r));
  });
};
